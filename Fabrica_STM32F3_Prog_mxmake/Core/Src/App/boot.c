...
/**************************************************************************************
** \brief     Receives the CONNECT request from the host, which indicates that
*the
...
#if (BOOT_COM_RS232_ENABLE > 0)
/****************************************************************************************
 *     U N I V E R S A L   A S Y N C H R O N O U S   R X   T X   I N T E R F A C
 *E
 ****************************************************************************************/
...
/** \brief Maximum bytes in a CTO packet on RS232, excluding the one extra for
 * length and two extra for possibly configured checksum byte(s).
 */
  ...
  if (xcpCtoRxInProgress == 0) {
    ...
    if (Rs232ReceiveByte(&xcpCtoReqPacket[xcpCtoRxLength + 1]) == 1) {
      ...
      if (TimerGet() > (xcpCtoRxStartTime + RS232_CTO_RX_PACKET_TIMEOUT_MS)) {
        /* cancel cto packet reception due to timeout. note that this
         * automatically discards the already received packet bytes, allowing
         * the host to retry.
         */
        xcpCtoRxInProgress = 0;
      }
...
** \param     prescaler Pointer to where the value for the prescaler will be
*stored.
...
**
****************************************************************************************/
static unsigned char CanGetSpeedConfig(unsigned short baud,
...
static void BootComCanInit(void) {
  ...
  /* determine the reception filter mask and id values such that it only leaves
   * one CAN identifier through (BOOT_COM_CAN_RX_MSG_ID).
   */
   ...
   * function always returns HAL_OK, so no need to evaluate the return value.
   */
...
static void BootComCanCheckActivationRequest(void) {
  ...
  if (HAL_CAN_GetRxMessage(&canHandle, CAN_RX_FIFO0, &rxMsgHeader, rxMsgData) ==
      HAL_OK) {
    /* check if this message has the configured CAN packet identifier. */
    if ((rxMsgId & 0x80000000) == 0) {
      /* was an 11-bit CAN message received that matches? */
      if ((rxMsgHeader.StdId == rxMsgId) && (rxMsgHeader.IDE == CAN_ID_STD)) {
      ...
      if ((rxMsgHeader.ExtId == rxMsgId) && (rxMsgHeader.IDE == CAN_ID_EXT)) {
        /* set flag that a packet with a matching CAN identifier was received.
         */
        packetIdMatches = 1;
      }
